...
ALTER TYPE gpc_product_status ADD VALUE IF NOT EXISTS 'archived';
...
CREATE TABLE IF NOT EXISTS gpc_manufacturers (
...
CREATE TABLE IF NOT EXISTS gpc_product_types (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  code text NOT NULL,
  name text NOT NULL,
...
CREATE TABLE IF NOT EXISTS gpc_product_cards (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS gpc_gtins (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE UNIQUE INDEX IF NOT EXISTS gpc_gtins_one_primary_per_product_idx
  ON gpc_gtins (product_card_id)
  WHERE is_primary;

CREATE TABLE IF NOT EXISTS gpc_attachments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  product_card_id uuid NOT NULL REFERENCES gpc_product_cards(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS gpc_validations (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  product_card_id uuid NOT NULL REFERENCES gpc_product_cards(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS gpc_audit_logs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS gpc_product_cards_manufacturer_id_idx ON gpc_product_cards (manufacturer_id);
CREATE INDEX IF NOT EXISTS gpc_product_cards_product_type_id_idx ON gpc_product_cards (product_type_id);
CREATE INDEX IF NOT EXISTS gpc_product_cards_status_idx ON gpc_product_cards (status);
CREATE INDEX IF NOT EXISTS gpc_product_cards_validation_status_idx ON gpc_product_cards (validation_status);
CREATE INDEX IF NOT EXISTS gpc_product_cards_technical_parameters_gin_idx ON gpc_product_cards USING gin (technical_parameters);
CREATE INDEX IF NOT EXISTS gpc_gtins_product_card_id_idx ON gpc_gtins (product_card_id);
CREATE INDEX IF NOT EXISTS gpc_attachments_product_card_id_idx ON gpc_attachments (product_card_id);
CREATE INDEX IF NOT EXISTS gpc_validations_product_card_id_created_at_idx ON gpc_validations (product_card_id, created_at DESC);
CREATE INDEX IF NOT EXISTS gpc_audit_logs_product_card_id_occurred_at_idx ON gpc_audit_logs (product_card_id, occurred_at DESC);
...
DROP TRIGGER IF EXISTS gpc_manufacturers_set_updated_at ON gpc_manufacturers;
...
DROP TRIGGER IF EXISTS gpc_product_types_set_updated_at ON gpc_product_types;
CREATE TRIGGER gpc_product_types_set_updated_at
BEFORE UPDATE ON gpc_product_types
FOR EACH ROW EXECUTE FUNCTION gpc_set_updated_at();

DROP TRIGGER IF EXISTS gpc_product_cards_set_updated_at ON gpc_product_cards;
